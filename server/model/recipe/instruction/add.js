  ...
  if (instruction && !isNaN(num_step)) {
    ...
    if (accountFromToken) {
      ...
      if ((await Misc.isSlugExist(slug)) && isMine) {
        await Misc.instruction.insert(slug, num_step)

        response.instructionById = await Db.commit({
          query: 'INSERT INTO `recipes_instructions` SET ?',
        ...
        response.toastMessage.push({
          type: 'success',
          msg: `Instruction ajouté en position ${num_step}`,
        })
